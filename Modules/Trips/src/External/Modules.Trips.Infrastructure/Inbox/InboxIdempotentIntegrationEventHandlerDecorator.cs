...
namespace Modules.Trips.Infrastructure.Inbox
{
    ...
    where TIntegrationEvent : IIntegrationEvent
    {
        public async override Task HandleAsync(TIntegrationEvent notification, CancellationToken cancellationToken = default)
        {
            ...
            const string query = $"SELECT COUNT(1) FROM {Schema.Trips}.inbox_consumer_messages WHERE id = @id AND handler_name = @HandlerName";
            ...
            const string insertQuery = $"INSERT INTO {Schema.Trips}.inbox_consumer_messages (id, handler_name) VALUES (@Id, @HandlerName)";
