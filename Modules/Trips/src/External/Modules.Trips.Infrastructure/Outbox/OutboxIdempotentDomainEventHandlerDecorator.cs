    ...
    where TDomainEvent : IDomainEvent
{
    public async Task HandleAsync(TDomainEvent notification, CancellationToken cancellationToken)
    {
        ...
        const string query = $"SELECT COUNT(1) FROM {Schema.Trips}.outbox_consumer_messages WHERE id = @id AND handler_name = @HandlerName";
        ...
        const string insertQuery = $"INSERT INTO {Schema.Trips}.outbox_consumer_messages (id, handler_name) VALUES (@Id, @HandlerName)";
